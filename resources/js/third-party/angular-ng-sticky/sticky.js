  ...
  module.directive('sticky', function() {
    ...
    function linkFn($scope, $elem, $attrs) {
      var mediaQuery, stickyClass, bodyClass, elem, $window, $body,
        ...
        stickyLine, stickyBottomLine, offset, anchor, confine, prevOffset, matchMedia, usePlaceholder, placeholder, bottomLineElem, initialParentOffset;
      ...
      doc     = document.documentElement;
			bottomLineElem = $attrs.bottomLine ? document.querySelector($attrs.bottomLine) : null;
      ...
      anchor = typeof $attrs.anchor === 'string' ?
        ...
        : 'top';
			...
			if(confine && bottomLineElem) {
				var body = document.body;
				var prevBodyHeight = body.offsetHeight;
				var _timeout = -1;

				$scope.$watchGroup([
						function() { return body.offsetHeight; }
				],  function(values) {
					//when a resize is happening, we calculate twice ,
					//to keep moving until we know exactly the new size
					recalculateBottomLine();
					clearTimeout(_timeout);
					setTimeout(recalculateBottomLine , 1000);

					function recalculateBottomLine() {
						var height = document.body.offsetHeight;
						var diffHeight = height - prevBodyHeight;
						prevBodyHeight = height;
						if(!stickyBottomLine) {
							calculateBottomLine();
						}
						else {
							stickyBottomLine  += diffHeight;
						}
						onResize();
					}
				});
      ...
      $scope.$watch( function() { // triggered on load and on digest cycle
        ...
        if ( newVal !== oldVal || typeof stickyLine === 'undefined' ) {
					if(typeof stickyLine === "undefined")
          ...
          // specify the bottom out line for the sticky to unstick
					...
					if(typeof stickyBottomLine === "undefined") {
						if(bottomLineElem) {
							calculateBottomLine();
						}
						else {
      ...
      //
			...
			function calculateBottomLine() {
				...
				initialParentOffset = $elem.parent()[0].getBoundingClientRect().top + curScroll;

				var marginBottom = parseInt($elem.css('margin-bottom').replace(/px;?/, '')) || 0;

				//stickyBottomLine, bottomLineElem, elem are from an outer scope
				stickyBottomLine = bottomLineElem.getBoundingClientRect().top - (elem.getBoundingClientRect().top + elem.offsetHeight + marginBottom);
			}
        ...
        if ( shouldStick && !_shouldStickWithLimit($attrs.stickLimit) && !isSticking) {
          stickElement();
            ...
            if(closest == stickyLine){
                from = 'top';
            } else if(closest == stickyBottomLine && bottomLineElem){
                from = 'element';
      ...
      function unstickElement(fromDirection, scrollTop) {
        ...
        if (fromDirection == 'top' ) {
                ...
                } else if (fromDirection == 'element' && confine === true) {
										var curScroll = document.documentElement.scrollTop || document.body.scrollTop,
												topPos = curScroll - initialParentOffset + offset;
												//topPos = bottomLineElem.getBoundingClientRect().top - initialParentOffset - elem.offsetHeight - offset 0 marginBottom;
                    $elem
                        .css('top', topPos+'px')
                        .css('bottom', '')
												.css('left', initialCSS.cssLeft)
                        .css('position', 'absolute');
										var fixOffset = bottomLineElem.getBoundingClientRect().top - elem.getBoundingClientRect().top - elem.offsetHeight;
										if(fixOffset < 0) {
											$elem
													.css('top', (topPos+fixOffset)+'px');
